...
public class Friend {
    ...
    public boolean connect() {
        ...
        try {
            ...
            String proofStr = UUID.randomUUID().toString();
            FullHttpRequest peerTrustTest = new DefaultFullHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.POST, "/p2p-api/keypair/sign", Unpooled.wrappedBuffer(proofStr.getBytes(StandardCharsets.ISO_8859_1)));
            peerTrustTest.headers().set(HttpHeaderNames.CONTENT_LENGTH, proofStr.getBytes(StandardCharsets.UTF_8).length);
            var trustTestResp = JsonUtil.standard().fromJson(extractString(proxier.send(peerTrustTest).join().content()), StdResp.class);
            var signedProofStr = Base64.getDecoder().decode((String) trustTestResp.data());
            if (!Ed25519Kt.unmarshalEd25519PublicKey(pubKey).verify(proofStr.getBytes(StandardCharsets.ISO_8859_1), signedProofStr)) {
    ...
    private String extractString(ByteBuf buf) {
        return buf.getCharSequence(0, buf.readableBytes(), StandardCharsets.UTF_8).toString();
    }
