...
public final class Identity {
    ...
    private static final String DEFAULT_PREFIX = "-PBH001-";
    ...
    public static Identity generate(String peerId, InetSocketAddress inetSocketAddress) {
        var random = ThreadLocalRandom.current();
        var buffer = ByteBuffer.allocate(20).put(peerId.getBytes(StandardCharsets.ISO_8859_1));
        while (buffer.hasRemaining()) {
            buffer.put((byte) random.nextInt('0', '9' + 1));
        }
        return new Identity(buffer.array(), inetSocketAddress);
    }
