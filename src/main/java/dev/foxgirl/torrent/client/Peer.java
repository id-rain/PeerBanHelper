...
public class Peer implements Protocol.Listener, AutoCloseable {
    ...
    public void onReceive(@NotNull MessageImpl message) throws Exception {
        synchronized (lock) {
            switch (message.getType()) {
                ...
                case REQUEST -> {
                    LOGGER.debug("Peer {} received request packet");
                }
                case PIECE -> {
                    LOGGER.debug("Peer {} received piece packet");
                }
                case SUGGEST_PIECE -> {
                    LOGGER.debug("Peer {} received suggest piece packet");
                }
                default -> {
                    //LOGGER.debug();
                }
